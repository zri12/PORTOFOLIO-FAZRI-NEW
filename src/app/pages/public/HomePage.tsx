...
import { ArrowRight, ArrowUpRight, Award, BriefcaseBusiness, Camera, Check, ChevronRight, Code2, Download, Github, Instagram, Linkedin, Mail, MapPin, MessageCircle, Monitor, MousePointer2, Palette, Smartphone, Sparkles, Wrench, X, Youtube } from "lucide-react";
...
export default function HomePage() {
  ...
  const contactChannels = [
    ...
    { icon: Smartphone, label: "WhatsApp", value: t("Start Chat"), href: profile.whatsapp ? `https://wa.me/${profile.whatsapp.replace(/\D/g, "")}` : "", meta: "Quick introduction" },
  ...
  return (
    ...
    <section className="relative overflow-hidden border-y border-[var(--color-border)] bg-[var(--color-surface-elevated)] px-6 py-24">
      ...
      <div className="relative mx-auto max-w-[1440px]">
        <Reveal>
          <Eyebrow>{t("08 / Recognition")}</Eyebrow>
    ...
    <section className="px-6 py-24"><div className="mx-auto max-w-7xl"><Reveal><Eyebrow>{t("Start a conversation")}</Eyebrow><h2 className="max-w-2xl font-manrope text-3xl font-bold">{t("Have a thoughtful project in mind?")}</h2><p className="mt-4 max-w-md leading-7 text-[var(--color-text-secondary)]">{profile.availability}</p></Reveal><div className="mt-10 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">{contactChannels.map(({ icon: Icon, label, value, href, meta }, index) => <Reveal key={label}><a href={href} target={href.startsWith("http") ? "_blank" : undefined} rel="noreferrer" className="group relative block min-h-[190px] overflow-hidden border border-[var(--color-border)] bg-[var(--color-surface-elevated)] p-5 transition-[transform,border-color,background-color,box-shadow] duration-300 hover:-translate-y-1 hover:border-[var(--color-accent-main)]/70 hover:bg-[var(--color-bg-secondary)]"><span className="absolute right-5 top-5 font-mono text-[10px] text-[var(--color-text-muted)]">{String(index + 1).padStart(2, "0")}</span><span className="flex h-12 w-12 items-center justify-center border border-[var(--color-accent-main)]/35 bg-[var(--color-accent-main)]/10 text-[var(--color-accent-main)]"><Icon size={22} /></span><p className="mt-7 font-mono text-[9px] uppercase tracking-[.16em] text-[var(--color-text-muted)]">{t(meta)}</p><div className="mt-2 flex items-end justify-between gap-3"><div className="min-w-0"><p className="font-manrope text-xl font-bold">{t(label)}</p><p className="mt-1 truncate text-sm text-[var(--color-text-secondary)]">{value}</p></div><ArrowUpRight className="shrink-0 text-[var(--color-text-muted)] transition-[transform,color] duration-300 group-hover:-translate-y-1 group-hover:translate-x-1 group-hover:text-[var(--color-accent-main)]" size={18} /></div></a></Reveal>)}</div>{isPublicFeatureEnabled(data.settings, "comments") && <Reveal className="mt-6 border border-[var(--color-border)] bg-[var(--color-surface-elevated)] p-8"><div className="flex items-start justify-between"><div><Eyebrow>{t("Guestbook")} / {comments.length} {t("notes")}</Eyebrow><h2 className="font-manrope text-3xl font-bold">{t("Recent visitor comments")}</h2></div><MessageCircle className="text-[var(--color-accent-main)]" /></div><div className="mt-8 space-y-5">{comments.filter((item) => item.status === "approved").slice(0, 3).map((item) => <div key={item.id} className="border-b border-[var(--color-border)] pb-5 last:border-0"><div className="flex items-center gap-2"><span className="flex h-7 w-7 items-center justify-center rounded-full bg-[var(--color-accent-main)]/15 text-xs font-bold text-[var(--color-accent-main)]">{item.avatar}</span><p className="text-sm font-semibold">{item.name}</p></div><p className="mt-2 pl-9 text-sm leading-6 text-[var(--color-text-secondary)]">"{item.message}"</p></div>)}</div><Link to="/contact" className="mt-6 inline-flex items-center gap-2 text-sm font-semibold text-[var(--color-accent-main)]">{t("Visit Contact & Guestbook")} <ArrowRight size={15} /></Link></Reveal>}</div></section>
