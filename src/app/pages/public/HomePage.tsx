...
export default function HomePage() {
  ...
  return (
    ...
    <section className="hero-section-shell relative isolate h-[100svh] min-h-[720px] overflow-hidden border-b border-[var(--color-border)]">
      ...
      <div className="relative z-10 flex h-full items-center overflow-hidden px-6 pb-12 pt-28 lg:pt-24">
        ...
        <div className="hero-main-grid relative z-10 mx-auto grid w-full max-w-7xl items-center gap-12 lg:grid-cols-[.93fr_1.07fr]">
          <motion.div initial={reduce ? false : { opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: .8 }} className="relative z-10 max-w-2xl">
            ...
            <h1 className="max-w-xl font-manrope text-5xl font-extrabold leading-[.99] tracking-[-.035em] md:text-6xl xl:text-7xl">Creative Web <span className="text-[var(--color-accent-main)]">Developer</span></h1>
            <p className="mt-7 max-w-xl text-lg leading-8 text-[var(--color-text-secondary)]">I create modern web applications and interactive experiences that combine reliable functionality with thoughtful visual design.</p>
          ...
          <motion.div
            ...
            className="hero-image-stage relative hidden min-h-[470px] w-full lg:block lg:min-h-[560px]"
    ...
    <section id="about" className="relative bg-[var(--color-bg-secondary)] px-6 py-24 lg:py-32"><div className="mx-auto max-w-7xl"><Reveal><Eyebrow>01 / About me</Eyebrow><div className="mb-14 max-w-2xl"><h2 className="font-manrope text-4xl font-bold tracking-[-.025em] md:text-5xl">Transforming ideas into functional and engaging digital experiences.</h2></div></Reveal><div className="grid gap-12 lg:grid-cols-[1fr_.85fr] lg:gap-20"><Reveal><p className="font-mono text-[10px] uppercase tracking-[.16em] text-[var(--color-accent-main)]">Hello, I'm</p><h3 className="mt-3 font-manrope text-3xl font-bold">Fazri Lukman Nurrohman</h3><p className="mt-6 max-w-xl leading-8 text-[var(--color-text-secondary)]">I focus on building modern, responsive, and functional web applications. My experience in interface design, photography, videography, and editing helps me create digital products that are technically clear, visually engaging, and effective in communicating ideas.</p><blockquote className="mt-8 max-w-xl border-l-2 border-[var(--color-accent-main)] bg-white/[.025] px-5 py-4 text-lg leading-8 text-[var(--color-accent-silver)]">"Technology becomes more meaningful when functionality, design, and visual storytelling work together."</blockquote><div className="mt-8 flex flex-wrap gap-3"><a href="#cv" className="inline-flex items-center gap-2 rounded-lg bg-[var(--color-text-main)] px-4 py-3 text-sm font-bold text-[var(--color-bg-primary)]"><Download size={15} />Download CV</a><Link to="/projects" className="inline-flex items-center gap-2 px-4 py-3 text-sm font-semibold text-[var(--color-text-secondary)] hover:text-white">View Projects <ArrowRight size={15} /></Link></div><div className="mt-9 flex flex-wrap gap-2 text-xs text-[var(--color-text-muted)]"><span className="border border-[var(--color-border)] px-3 py-2">Creative Web Developer</span><span className="border border-[var(--color-border)] px-3 py-2">Indonesia</span><span className="border border-[var(--color-border)] px-3 py-2">Available for selected projects</span></div></Reveal><Reveal className="relative min-h-[390px]"><div className="absolute inset-0 overflow-hidden border border-[var(--color-border)] bg-[linear-gradient(135deg,var(--color-surface-elevated),transparent)]"><img src={aboutImage} alt="Professional portrait of Fazri" className="h-full w-full object-contain object-center opacity-85 grayscale transition duration-700 hover:scale-105 hover:grayscale-0" /></div><div className="absolute -bottom-5 -left-5 max-w-[210px] border border-[var(--color-border)] bg-[var(--color-bg-primary)] p-4"><p className="font-mono text-[9px] uppercase tracking-[.16em] text-[var(--color-accent-main)]">Creative support</p><p className="mt-2 text-sm leading-6 text-[var(--color-text-secondary)]">Design, photography, video and editing enhance every build.</p></div></Reveal></div><div className="mt-20 grid gap-px overflow-hidden border border-[var(--color-border)] bg-[var(--color-border)] md:grid-cols-3">{[["10+", "Total Projects", "Digital systems and web experiences", BriefcaseBusiness], ["8+", "Certificates", "Continuous learning and practical studies", Award], ["2+", "Years of Experience", "From academic work to client delivery", Sparkles]].map(([value, label, desc, Icon]) => <Reveal key={String(label)} className="bg-[var(--color-bg-secondary)] p-7 transition hover:bg-[var(--color-surface-elevated)]"><Icon className="mb-12 text-[var(--color-accent-main)]" size={21} /><strong className="block font-manrope text-4xl">{String(value)}</strong><p className="mt-2 font-semibold">{String(label)}</p><p className="mt-2 text-sm text-[var(--color-text-muted)]">{String(desc)}</p><div className="mt-6 h-px bg-[var(--color-border)]"><span className="block h-px w-3/4 bg-[var(--color-accent-main)]" /></div></Reveal>)}</div><div className="mt-6 grid gap-4 text-sm text-[var(--color-text-secondary)] md:grid-cols-3"><p><span className="text-[var(--color-text-muted)]">Main Focus:</span> Web Development</p><p><span className="text-[var(--color-text-muted)]">Current Learning:</span> Web Animation & 3D</p><p><span className="text-[var(--color-text-muted)]">Creative Support:</span> Design, Photo, Video</p></div></div></section>
    ...
    <section className="bg-[var(--color-surface-elevated)] px-6 py-24"><div className="mx-auto max-w-7xl"><Reveal><Eyebrow>09 / Recognition</Eyebrow><div className="flex flex-wrap items-end justify-between gap-5"><h2 className="font-manrope text-4xl font-bold">Certificates & achievements</h2><Link to="/certificates" className="text-sm font-semibold text-[var(--color-accent-main)]">View certificate archive <ArrowRight className="inline" size={15} /></Link></div></Reveal><div className="mt-12 grid gap-4 lg:grid-cols-[1.3fr_.7fr_.7fr]">{[["Web Development Fundamentals", "Dicoding", "2024", "Professional Development"], ["UI/UX Design Essentials", "Coursera", "2023", "Design"], ["JavaScript Intermediate", "Codepolitan", "2023", "Development"]].map(([title, issuer, year, category], index) => <Reveal key={title} className={`group relative min-h-[230px] overflow-hidden border border-[var(--color-border)] ${index === 0 ? "bg-[linear-gradient(135deg,#d8e7e8,#6d8795)] text-slate-950" : "bg-[var(--color-bg-primary)]"} p-6`}><div className="absolute right-6 top-5 text-5xl font-black opacity-10">FL</div><Award size={24} className={index === 0 ? "text-slate-800" : "text-[var(--color-accent-main)]"} /><p className={`mt-12 font-mono text-[9px] uppercase tracking-[.16em] ${index === 0 ? "text-slate-700" : "text-[var(--color-text-muted)]"}`}>{category}</p><h3 className="mt-2 font-manrope text-xl font-bold">{title}</h3><p className={`mt-2 text-sm ${index === 0 ? "text-slate-700" : "text-[var(--color-text-secondary)]"}`}>{issuer} - {year}</p><button className="mt-6 text-xs font-bold underline underline-offset-4">View Certificate</button></Reveal>)}</div></div></section>
