...
import {
  ...
  localizeExperience,
  localizeProfile,
  publishedCertificatesForLanguage,
  publishedCreativeWorksForLanguage,
  publishedProjectsForLanguage,
...
import { isPublicFeatureEnabled, shouldLoadThree } from "../../lib/publicSettings";
...
function HomeCertificateCard({ certificate, index, onView }: { certificate: Certificate; index: number; onView: (id: string) => void }) {
  ...
  return (
    <button
      ...
      className="group block h-full w-full text-left text-[var(--color-text-main)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--color-accent-main)] focus-visible:ring-offset-4 focus-visible:ring-offset-[var(--color-surface-elevated)]"
        ...
        <div className="relative flex h-[340px] shrink-0 items-center justify-center overflow-hidden border-b border-[var(--color-border)] bg-[radial-gradient(circle_at_50%_18%,rgba(78,187,232,.14),transparent_52%),linear-gradient(145deg,var(--color-bg-secondary),var(--color-bg-primary))] p-5 sm:p-7">
          ...
          {hasImage ? (
            <img
              ...
              aria-hidden="true"
              loading="lazy"
              className="relative z-10 max-h-full max-w-full object-contain drop-shadow-[0_22px_38px_rgba(0,0,0,.5)] transition duration-500 group-hover:scale-[1.035]"
              onError={() => setImageFailed(true)}
            />
          ) : (
            <div className="relative z-10 flex aspect-[4/3] h-full max-h-[270px] w-full max-w-[390px] flex-col border border-[var(--color-border)] bg-[linear-gradient(145deg,rgba(78,187,232,.1),rgba(255,255,255,.025))] p-7 shadow-[0_18px_45px_rgba(0,0,0,.24)]">
              <div className="flex items-start justify-between">
                <Award size={28} className="text-[var(--color-accent-main)]" />
                <span className="font-manrope text-4xl font-black text-white/[.045]">FL</span>
              </div>
              <div className="mt-auto">
                <p className="font-mono text-[9px] uppercase tracking-[.2em] text-[var(--color-accent-main)]">{certificate.category}</p>
                <p className="mt-3 line-clamp-2 font-manrope text-xl font-bold leading-tight text-[var(--color-text-main)]">{certificate.title}</p>
                <div className="mt-5 grid gap-2">
                  <span className="block h-px w-full bg-[var(--color-border)]" />
                  <span className="block h-px w-3/4 bg-[var(--color-border)]" />
                </div>
              </div>
            </div>
          )}
        ...
        <div className="flex flex-1 flex-col p-6 sm:p-7">
          <div className="flex items-center justify-between gap-4">
            <div className="flex min-w-0 items-center gap-3">
              ...
              {isFeatured && <span className="shrink-0 border border-[var(--color-accent-main)]/35 bg-[var(--color-accent-main)]/10 px-2 py-1 font-mono text-[8px] uppercase tracking-[.14em] text-[var(--color-accent-main)]">{t("Featured")}</span>}
            </div>
          ...
          <p className="mt-4 text-sm text-[var(--color-text-secondary)]">{certificate.issuer || t("Independent learning")}</p>
          ...
          <span className="mt-auto flex items-center justify-between border-t border-[var(--color-border)] pt-6 text-xs font-bold text-[var(--color-text-main)] transition group-hover:text-[var(--color-accent-main)]">
            {t("View Certificate")}
            <span className="flex h-9 w-9 items-center justify-center border border-[var(--color-border)] transition duration-300 group-hover:border-[var(--color-accent-main)] group-hover:bg-[var(--color-accent-main)] group-hover:text-[var(--color-bg-primary)]"><ArrowRight size={15} /></span>
          </span>
        </div>
      </article>
    </button>
...
function CertificatePreviewModal({ certificate, onClose }: { certificate: Certificate; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-[120] flex items-center justify-center bg-black/82 p-3 backdrop-blur-sm sm:p-5" role="dialog" aria-modal="true" aria-label={`${t("Certificate preview")}: ${certificate.title}`}>
      <div className="flex h-[calc(100svh-1.5rem)] w-full max-w-6xl flex-col overflow-hidden border border-[var(--color-border)] bg-[var(--color-bg-secondary)] shadow-2xl sm:h-[calc(100svh-2.5rem)]">
        <div className="flex shrink-0 items-center justify-between gap-4 border-b border-[var(--color-border)] px-4 py-3 sm:px-5">
          ...
          <button type="button" onClick={onClose} className="shrink-0 p-2 text-[var(--color-text-muted)] hover:text-[var(--color-text-main)]" aria-label={t("Close certificate preview")}><X size={20} /></button>
        </div>
        <div className="grid min-h-0 flex-1 grid-rows-[minmax(0,1fr)_auto] bg-[var(--color-bg-primary)] lg:grid-cols-[1fr_320px] lg:grid-rows-1">
          ...
          <aside className="shrink-0 border-t border-[var(--color-border)] bg-[var(--color-bg-secondary)] p-5 lg:border-l lg:border-t-0">
            ...
            <div className="mt-5 space-y-4 text-sm text-[var(--color-text-secondary)]">
              <p><span className="block text-[var(--color-text-muted)]">{t("Issuer")}</span>{certificate.issuer || t("Independent learning")}</p>
              {year && <p><span className="block text-[var(--color-text-muted)]">{t("Year")}</span>{year}</p>}
              {certificate.credentialId && <p><span className="block text-[var(--color-text-muted)]">{t("Credential ID")}</span>{certificate.credentialId}</p>}
            </div>
            {certificate.credentialUrl && <a href={certificate.credentialUrl} target="_blank" rel="noreferrer" className="mt-6 inline-flex items-center gap-2 border-b border-[var(--color-accent-main)] pb-2 text-xs font-bold text-[var(--color-accent-main)]">{t("Open credential")} <ArrowRight size={13} /></a>}
          </aside>
        </div>
      </div>
    </div>
...
export default function HomePage() {
  ...
  const projectsData = publishedProjectsForLanguage(Array.isArray(data.projects) ? data.projects : [], language);
  const profile = localizeProfile(data.profile, language);
  const cvLink = profile.cvUrl.trim() ? {
    href: profile.cvUrl,
    target: /^https?:\/\//i.test(profile.cvUrl) ? "_blank" : undefined,
    rel: "noopener noreferrer",
  } : { href: undefined, hidden: true };
  const techStack = Array.isArray(data.techStack) ? data.techStack : [];
  
  const certificates = publishedCertificatesForLanguage(Array.isArray(data.certificates) ? data.certificates : [], language);
  const creativeWorks = publishedCreativeWorksForLanguage(Array.isArray(data.creativeWorks) ? data.creativeWorks : [], language)
    .sort((a, b) => Number(b.featured) - Number(a.featured) || a.displayOrder - b.displayOrder);
  const experiences = (Array.isArray(data.experiences) ? data.experiences : [])
    .filter((experience) => experience.published && hasExperienceLanguage(experience, language))
    .map((experience) => localizeExperience(experience, language))
  ...
  const creativeStackItems = creativeWorks.map((work) => ({ id: work.id, imageSrc: work.cover, href: `/creative-works/${work.slug}`, tag: work.category, title: work.title, description: work.description }));
  ...
  return (
    ...
    <section className="hero-section-shell relative isolate min-h-[100svh] overflow-hidden border-b border-[var(--color-border)] lg:h-[100svh] lg:min-h-[720px]">
      ...
      {shouldLoadThree(data.settings, isSpiderMode, spiderSceneReady, compactViewport) && (
      ...
      <div className="hero-layout relative z-10 flex min-h-[100svh] items-center overflow-hidden px-5 pb-0 pt-24 sm:px-6 sm:pt-28 lg:h-full lg:min-h-0 lg:pb-12 lg:pt-24">
        ...
        <div className="hero-main-grid relative z-10 mx-auto grid w-full max-w-7xl items-center gap-6 sm:gap-8 lg:grid-cols-[.93fr_1.07fr] lg:gap-12">
          <motion.div initial={reduce ? false : { opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: .8 }} className="hero-copy relative z-20 max-w-2xl">
            <div className="mb-7 flex items-center gap-3"><span className="h-2 w-2 rounded-full bg-[var(--color-accent-secondary)] shadow-[0_0_18px_var(--color-accent-secondary)]" /><span className="font-mono text-[10px] uppercase tracking-[.18em] text-[var(--color-text-secondary)]">{profile.greeting}</span></div>
            <p className="mb-3 font-manrope text-base font-semibold text-[var(--color-text-secondary)]">{profile.fullName}</p>
            <h1 className="max-w-xl font-manrope text-[2.55rem] font-extrabold leading-[.98] tracking-[-.035em] sm:text-5xl md:text-6xl xl:text-7xl">{profile.title}</h1>
            <p className="mt-6 max-w-xl text-base leading-7 text-[var(--color-text-secondary)] sm:mt-7 sm:text-lg sm:leading-8">{profile.headline}</p>
            <p className="mt-3 max-w-lg text-sm leading-6 text-[var(--color-text-muted)]">{profile.description}</p>
            ...
            <div className="hero-actions mt-7 flex flex-wrap gap-3 sm:mt-8">
              <Link to="/projects" className={`inline-flex items-center gap-2 px-5 py-3 text-sm font-bold transition-transform hover:-translate-y-0.5 ${mode === 'spider' ? 'bg-[var(--color-primary)] text-white hover:shadow-[0_0_15px_rgba(227,33,60,0.4)] border border-red-500/50' : 'rounded-lg bg-[var(--color-text-main)] text-[var(--color-bg-primary)]'}`}>{t("Explore Projects")} <ArrowRight size={16} /></Link>
              {isPublicFeatureEnabled(data.settings, "contact") && <Link to="/contact" className={`inline-flex items-center gap-2 px-5 py-3 text-sm font-semibold hover:bg-white/5 ${mode === 'spider' ? 'border border-[var(--color-border)] text-white' : 'rounded-lg border border-[var(--color-border)]'}`}>{t("Contact Me")}</Link>}
              <a {...cvLink} className="inline-flex items-center gap-2 px-3 py-3 text-sm font-semibold text-[var(--color-text-secondary)] hover:text-[var(--color-accent-main)]"><Download size={15} /> {t("Download CV")}</a>
            </div>
            <div className="hero-meta mt-8 flex flex-wrap items-center gap-x-5 gap-y-2 text-xs text-[var(--color-text-muted)] sm:mt-10"><span className="inline-flex items-center gap-1.5"><MapPin size={13} />{profile.location}</span><span className="inline-flex items-center gap-1.5"><Check size={13} />{profile.availability}</span>{socialLinks.map((link) => <a key={link.label} href={link.href} target="_blank" rel="noreferrer" className="hover:text-[var(--color-text-main)]">{link.label}</a>)}</div>
          </motion.div>
          <motion.div
          ...
          <motion.div initial={false} animate={{ opacity: 0 }} className="hidden" aria-hidden="true">
            ...
            <div ref={heroModeCardRef} className={`hero-mode-card absolute bottom-[3%] right-[3%] border border-white/10 bg-[var(--color-bg-secondary)]/90 px-4 py-3 backdrop-blur z-20 transition-all duration-500 ${mode === 'spider' ? 'mode-switch-spider' : ''}`}>
              <p className="font-mono text-[9px] uppercase tracking-[.16em] text-[var(--color-text-muted)]">{t("Now exploring")}</p>
        ...
        <div className="absolute bottom-8 left-1/2 hidden -translate-x-1/2 items-center gap-3 lg:flex"><span className="h-px w-12 bg-[var(--color-border)]" /><span className="font-mono text-[9px] tracking-[.18em] text-[var(--color-text-muted)]">{t("SCROLL TO EXPLORE")}</span><MousePointer2 size={13} className="text-[var(--color-accent-main)]" /></div>
      </div>
    </section>

    <section id="about" className="relative bg-[var(--color-bg-secondary)] px-5 py-20 sm:px-6 sm:py-24 lg:py-32"><div className="mx-auto max-w-7xl"><Reveal><Eyebrow>{t("01 / About me")}</Eyebrow><div className="mb-10 max-w-2xl sm:mb-14"><h2 className="font-manrope text-3xl font-bold tracking-[-.025em] sm:text-4xl md:text-5xl">{t("Transforming ideas into functional and engaging digital experiences.")}</h2></div></Reveal><div className="grid gap-12 lg:grid-cols-[1fr_.85fr] lg:gap-20"><Reveal><p className="font-mono text-[10px] uppercase tracking-[.16em] text-[var(--color-accent-main)]">{t("Hello, I'm")}</p><h3 className="mt-3 font-manrope text-2xl font-bold sm:text-3xl">Fazri Lukman Nurrohman</h3><p className="mt-6 max-w-xl leading-7 text-[var(--color-text-secondary)] sm:leading-8">{profile.biography}</p><blockquote className="mt-8 max-w-xl border-l-2 border-[var(--color-accent-main)] bg-white/[.025] px-5 py-4 text-base leading-7 text-[var(--color-accent-silver)] sm:text-lg sm:leading-8">{profile.aboutContent || profile.description}</blockquote><div className="mt-8 flex flex-wrap gap-3"><a {...cvLink} className="inline-flex items-center gap-2 rounded-lg bg-[var(--color-text-main)] px-4 py-3 text-sm font-bold text-[var(--color-bg-primary)]"><Download size={15} />{t("Download CV")}</a><Link to="/projects" className="inline-flex items-center gap-2 px-4 py-3 text-sm font-semibold text-[var(--color-text-secondary)] hover:text-white">{t("View Projects")} <ArrowRight size={15} /></Link></div><div className="mt-9 flex flex-wrap gap-2 text-xs text-[var(--color-text-muted)]"><span className="border border-[var(--color-border)] px-3 py-2">{profile.title}</span><span className="border border-[var(--color-border)] px-3 py-2">{profile.location}</span><span className="border border-[var(--color-border)] px-3 py-2">{profile.availability}</span></div></Reveal><Reveal className="about-portrait relative min-h-[390px]"><div className="absolute inset-0 overflow-hidden border border-[var(--color-border)] bg-[linear-gradient(135deg,var(--color-surface-elevated),transparent)]"><img src={aboutImage} onError={(event) => { event.currentTarget.src = portrait; }} alt={profile.fullName} className="h-full w-full object-contain object-center opacity-85 grayscale transition duration-700 hover:scale-105 hover:grayscale-0" /></div><div className="about-support-card absolute -bottom-5 -left-5 max-w-[210px] border border-[var(--color-border)] bg-[var(--color-bg-primary)] p-4"><p className="font-mono text-[9px] uppercase tracking-[.16em] text-[var(--color-accent-main)]">{t("Creative support")}</p><p className="mt-2 text-sm leading-6 text-[var(--color-text-secondary)]">{t("Design, photography, video and editing enhance every build.")}</p></div></Reveal></div><div className="mt-20 grid gap-px overflow-hidden border border-[var(--color-border)] bg-[var(--color-border)] md:grid-cols-3">{[["10+", "Total Projects", "Digital systems and web experiences", BriefcaseBusiness], ["8+", "Certificates", "Continuous learning and practical studies", Award], ["2+", "Years of Experience", "From academic work to client delivery", Sparkles]].map(([value, label, desc, Icon]) => <Reveal key={String(label)} className="bg-[var(--color-bg-secondary)] p-6 transition hover:bg-[var(--color-surface-elevated)] sm:p-7"><Icon className="mb-9 text-[var(--color-accent-main)] sm:mb-12" size={21} /><strong className="block font-manrope text-4xl">{String(value)}</strong><p className="mt-2 font-semibold">{t(String(label))}</p><p className="mt-2 text-sm text-[var(--color-text-muted)]">{t(String(desc))}</p><div className="mt-6 h-px bg-[var(--color-border)]"><span className="block h-px w-3/4 bg-[var(--color-accent-main)]" /></div></Reveal>)}</div><div className="mt-6 grid gap-4 text-sm text-[var(--color-text-secondary)] md:grid-cols-3"><p><span className="text-[var(--color-text-muted)]">{t("Main Focus:")}</span> {t("Web Development")}</p><p><span className="text-[var(--color-text-muted)]">{t("Current Learning:")}</span> {t("Web Animation & 3D")}</p><p><span className="text-[var(--color-text-muted)]">{t("Creative Support:")}</span> {t("Design, Photo, Video")}</p></div></div></section>

    <section className="relative px-6 py-24"><div className="mx-auto grid max-w-7xl gap-6 lg:grid-cols-12"><Reveal className="border border-[var(--color-border)] bg-[var(--color-surface-elevated)] p-8 lg:col-span-5"><Eyebrow>{t("02 / Snapshot")}</Eyebrow><p className="font-manrope text-2xl font-bold">{t("A growing practice, grounded in useful outcomes.")}</p><strong className="mt-12 block font-manrope text-7xl font-bold text-[var(--color-accent-main)]">16</strong><p className="mt-2 text-sm text-[var(--color-text-secondary)]">{t("Projects completed across web applications and creative production.")}</p></Reveal><Reveal className="grid gap-6 lg:col-span-7 md:grid-cols-2"><div className="border-b border-[var(--color-border)] pb-6"><span className="font-mono text-[10px] text-[var(--color-text-muted)]">{t("WEB APPLICATIONS")}</span><strong className="mt-3 block text-4xl">06</strong></div><div className="border-b border-[var(--color-border)] pb-6"><span className="font-mono text-[10px] text-[var(--color-text-muted)]">{t("CREATIVE WORKS")}</span><strong className="mt-3 block text-4xl">12</strong></div><div><span className="font-mono text-[10px] text-[var(--color-text-muted)]">{t("TECHNOLOGIES USED")}</span><strong className="mt-3 block text-4xl">20+</strong></div><div><span className="font-mono text-[10px] text-[var(--color-text-muted)]">{t("CURRENT AVAILABILITY")}</span><strong className="mt-3 block text-xl text-[var(--color-accent-secondary)]">{t("Open for select work")}</strong></div><div className="col-span-full flex items-center gap-3 border-t border-[var(--color-border)] pt-6"><span className="font-mono text-[9px] text-[var(--color-text-muted)]">{t("2024 ACTIVITY")}</span><div className="flex flex-1 gap-1">{Array.from({ length: 18 }).map((_, i) => <span key={i} className={`h-2 flex-1 ${i % 5 === 0 ? "bg-[var(--color-accent-secondary)]" : i % 3 === 0 ? "bg-[var(--color-accent-main)]/60" : "bg-white/10"}`} />)}</div></div></Reveal></div></section>

    <section className="bg-[var(--color-bg-secondary)] px-6 py-24"><div className="mx-auto max-w-7xl"><Reveal><Eyebrow>{t("03 / What I build")}</Eyebrow><div className="flex flex-wrap items-end justify-between gap-6"><h2 className="max-w-xl font-manrope text-4xl font-bold tracking-[-.025em]">{t("Development systems, with a visual point of view.")}</h2><Link to="/contact" className="inline-flex items-center gap-2 text-sm font-semibold text-[var(--color-accent-main)]">{t("Discuss a project")} <ArrowRight size={15} /></Link></div></Reveal><div className="mt-14 grid gap-4 lg:grid-cols-12"><Reveal className={`spider-service-panel relative flex min-h-[340px] flex-col justify-between overflow-hidden border border-[var(--color-border)] bg-[var(--color-surface-elevated)] p-8 lg:col-span-6 ${mode === 'spider' ? 'border-red-500/30' : ''}`}><Code2 size={30} className="text-[var(--color-accent-main)]" /><div><h3 className="font-manrope text-3xl font-bold">{t("Website & Web Application Development")}</h3><p className="mt-3 max-w-md leading-7 text-[var(--color-text-secondary)]">{t("Structured, responsive systems from public-facing web experiences to internal operations dashboards.")}</p><div className="mt-6 flex flex-wrap gap-x-4 gap-y-2 text-xs text-[var(--color-text-muted)]"><span>{t("Responsive implementation")}</span><span>{t("Database integration")}</span><span>{t("Deployment support")}</span></div></div></Reveal><div className="grid gap-4 sm:grid-cols-2 lg:col-span-6">{[[Monitor, "Frontend Development", "Interfaces that feel considered at every breakpoint."], [Wrench, "Dashboard Development", "Clarity for complex operations and data."], [Palette, "UI Implementation", "Design language translated faithfully into code."], [Camera, "Creative Production", "Photography, video and edits that complete the story."]].map(([Icon, title, description]) => <Reveal key={String(title)} className={`spider-service-panel relative overflow-hidden border border-[var(--color-border)] bg-[var(--color-bg-primary)] p-6 transition hover:border-[var(--color-accent-main)]/50 ${mode === 'spider' ? 'border-red-500/20' : ''}`}><Icon size={20} className="text-[var(--color-accent-main)]" /><h3 className="mt-10 font-manrope text-lg font-bold">{t(String(title))}</h3><p className="mt-2 text-sm leading-6 text-[var(--color-text-muted)]">{t(String(description))}</p><button className="mt-5 text-xs font-semibold text-[var(--color-text-secondary)] hover:text-[var(--color-accent-main)]">{t("View details")} <ChevronRight className="inline" size={13} /></button></Reveal>)}</div></div></div></section>

    <section className="px-6 py-24"><div className="mx-auto max-w-7xl"><Reveal><Eyebrow>{t("04 / Selected systems")}</Eyebrow><h2 className="font-manrope text-4xl font-bold">{t("Featured Web Projects")}</h2></Reveal><div className="mt-12 space-y-10">{projectsData.slice(0, 4).map((project, index) => <Reveal key={project.slug}><Link to={`/projects/${project.slug}`} className={`group grid items-center gap-8 ${index % 2 ? "lg:grid-cols-[.82fr_1.18fr]" : "lg:grid-cols-[1.18fr_.82fr]"}`}><div className={index % 2 ? "lg:order-2" : ""}><ProjectPreview slug={project.slug} className="aspect-[16/10] transition duration-500 group-hover:-translate-y-2 group-hover:shadow-[0_25px_70px_rgba(0,0,0,.4)]" /></div><div className={index % 2 ? "lg:order-1 lg:pl-10" : "lg:pr-10"}><p className="font-mono text-[10px] uppercase tracking-[.16em] text-[var(--color-accent-main)]">0{index + 1} / {project.category}</p><h3 className="mt-4 font-manrope text-3xl font-bold">{project.title}</h3><p className="mt-3 max-w-md leading-7 text-[var(--color-text-secondary)]">{project.shortDescription}</p><div className="mt-5 flex flex-wrap gap-2">{project.techStack.slice(0, 3).map((tech) => <span key={tech} className="border border-[var(--color-border)] px-2 py-1 font-mono text-[9px] text-[var(--color-text-muted)]">{tech}</span>)}</div><span className="mt-7 inline-flex items-center gap-2 text-sm font-semibold transition group-hover:text-[var(--color-accent-main)]">{t("View case study")} <ArrowRight size={16} /></span></div></Link></Reveal>)}</div><Link to="/projects" className="mt-14 inline-flex items-center gap-2 border-b border-[var(--color-accent-main)] pb-2 text-sm font-bold">{t("View all projects")} <ArrowRight size={16} /></Link></div></section>

    <section className="relative border-y border-[var(--color-border)] bg-[var(--color-surface-elevated)] px-6 py-24"><div className="mx-auto grid max-w-7xl gap-12 lg:grid-cols-[.8fr_1.2fr]"><Reveal><Eyebrow>{t("05 / Process")}</Eyebrow><h2 className="font-manrope text-4xl font-bold">{t("From Idea to Digital Product")}</h2><p className="mt-5 max-w-sm leading-7 text-[var(--color-text-secondary)]">{t("A collaborative progression that takes uncertainty into a clear, tested product.")}</p></Reveal><div className="grid gap-px border border-[var(--color-border)] bg-[var(--color-border)] sm:grid-cols-2">{["Discover", "Analyze", "Plan", "Design", "Develop", "Test", "Deploy", "Improve"].map((stage, index) => <Reveal key={stage} className="group bg-[var(--color-surface-elevated)] p-5 transition hover:bg-[var(--color-bg-secondary)]"><span className="font-mono text-[10px] text-[var(--color-accent-main)]">0{index + 1}</span><div className="mt-9 flex items-end justify-between"><h3 className="font-manrope text-xl font-bold">{t(stage)}</h3><span className="h-8 w-8 border border-[var(--color-border)] transition group-hover:border-[var(--color-accent-main)]" /></div></Reveal>)}</div></div></section>

    <section className="px-6 py-24"><div className="mx-auto max-w-7xl"><Reveal><Eyebrow>{t("06 / Toolkit")}</Eyebrow><div className="flex flex-wrap items-end justify-between gap-6"><h2 className="font-manrope text-4xl font-bold">{t("Technology Stack")}</h2><div className={`flex border ${mode === 'spider' ? 'border-red-500/30' : 'border-[var(--color-border)]'}`}>{(Object.keys(techGroups) as Array<keyof typeof techGroups>).map((tab) => <button key={tab} onClick={() => setTechTab(tab)} className={`px-4 py-2 text-xs font-semibold transition ${techTab === tab ? "bg-[var(--color-text-main)] text-[var(--color-bg-primary)]" : "text-[var(--color-text-muted)] hover:text-white"}`}>{t(tab)}</button>)}</div></div></Reveal><motion.div layout className={`relative mt-10 grid gap-px border bg-[var(--color-border)] grid-cols-2 sm:grid-cols-4 lg:grid-cols-8 ${mode === 'spider' ? 'border-red-500/30' : 'border-[var(--color-border)]'}`}>
    ...
    <section className="border-y border-white/5 bg-[#050505] px-6 py-24"><div className="mx-auto max-w-7xl"><Reveal><Eyebrow>{t("07 / Beyond the browser")}</Eyebrow><div className="flex flex-wrap items-end justify-between gap-5"><h2 className="font-manrope text-4xl font-bold">{t("Selected Creative Works")}</h2><Link to="/creative-works" className="inline-flex items-center gap-2 text-sm font-semibold text-[var(--color-accent-main)]">{t("Explore creative archive")} <ArrowRight size={15} /></Link></div></Reveal><Reveal className="mt-10"><CardStack items={creativeStackItems} cardWidth={660} cardHeight={405} maxVisible={5} className={mode === "spider" ? "spider-creative-card" : ""} /></Reveal></div></section>
    {experiences.length > 0 && <section className="px-6 py-24"><div className="mx-auto max-w-7xl"><Reveal><Eyebrow>{t("08 / Journey")}</Eyebrow><h2 className="font-manrope text-4xl font-bold">{t("Experience & ongoing practice")}</h2></Reveal><div className="mt-12 border-l border-[var(--color-border)] pl-6 lg:ml-[22%] lg:pl-12">{experiences.map((experience) => <Reveal key={experience.id} className="relative pb-12 last:pb-0"><span className="absolute -left-[31px] top-1 h-3 w-3 rounded-full border-2 border-[var(--color-bg-primary)] bg-[var(--color-accent-main)] lg:-left-[55px]" /><p className="font-mono text-[10px] text-[var(--color-accent-main)]">{experience.period}</p><div className="mt-3 grid gap-3 lg:grid-cols-[.7fr_1.3fr]"><div><h3 className="font-manrope text-xl font-bold">{experience.role}</h3><p className="mt-1 text-sm text-[var(--color-text-muted)]">{experience.organization}{experience.location ? ` — ${experience.location}` : ""}</p></div><div><p className="text-sm leading-6 text-[var(--color-text-secondary)]">{experience.description}</p>{experience.technologies.length > 0 && <div className="mt-3 flex flex-wrap gap-2">{experience.technologies.map((technology) => <span key={technology} className="border border-[var(--color-border)] px-2 py-1 font-mono text-[8px] text-[var(--color-text-muted)]">{technology}</span>)}</div>}</div></div></Reveal>)}</div></div></section>}
      ...
      <div className="relative mx-auto max-w-[1440px]">
        <Reveal>
          ...
          <div className="flex flex-wrap items-end justify-between gap-5">
            <div>
              <h2 className="font-manrope text-4xl font-bold tracking-[-.025em]">{t("Certificates & achievements")}</h2>
              <p className="mt-3 max-w-xl text-sm leading-6 text-[var(--color-text-secondary)]">{t("Selected learning records that strengthen the web development practice and keep the portfolio grounded in real progress.")}</p>
            </div>
            <Link to="/certificates" className="inline-flex items-center gap-2 border-b border-[var(--color-accent-main)] pb-2 text-sm font-semibold text-[var(--color-accent-main)]">{t("View certificate archive")} <ArrowRight size={15} /></Link>
          </div>
        </Reveal>
    ...
    <section className="px-6 py-24"><div className="mx-auto grid max-w-7xl gap-6 lg:grid-cols-2"><Reveal className="border border-[var(--color-border)] bg-[var(--color-bg-secondary)] p-8 relative overflow-hidden">
      ...
      <Eyebrow>{t("Start a conversation")}</Eyebrow><h2 className="font-manrope text-3xl font-bold">{t("Have a thoughtful project in mind?")}</h2><p className="mt-4 max-w-md leading-7 text-[var(--color-text-secondary)]">{profile.availability}</p><div className="mt-7 space-y-3 text-sm text-[var(--color-text-secondary)]"><a href={`mailto:${profile.email}`} className="flex items-center gap-3 hover:text-[var(--color-accent-main)]"><Mail size={16} />{profile.email}</a>{profile.whatsapp && <a href={`https://wa.me/${profile.whatsapp.replace(/\D/g, "")}`} target="_blank" rel="noreferrer" className="flex items-center gap-3 hover:text-[var(--color-accent-main)]"><MessageCircle size={16} />WhatsApp</a>}<div className="flex gap-3 pt-2">{socialLinks.map((link) => <a key={link.label} href={link.href} target="_blank" rel="noreferrer" aria-label={link.label}>{link.label === "GitHub" ? <Github size={17} /> : link.label === "LinkedIn" ? <Linkedin size={17} /> : <Instagram size={17} />}</a>)}</div></div>{isPublicFeatureEnabled(data.settings, "contact") && <Link to="/contact" className="mt-8 inline-flex w-max items-center gap-2 bg-[var(--color-text-main)] px-4 py-3 text-sm font-bold text-[var(--color-bg-primary)]">{t("Send inquiry")} <Send size={14} /></Link>}</Reveal>{isPublicFeatureEnabled(data.settings, "comments") && <Reveal className="border border-[var(--color-border)] bg-[var(--color-surface-elevated)] p-8"><div className="flex items-start justify-between"><div><Eyebrow>{t("Guestbook")} / {comments.length} {t("notes")}</Eyebrow><h2 className="font-manrope text-3xl font-bold">{t("Recent visitor comments")}</h2></div><MessageCircle className="text-[var(--color-accent-main)]" /></div><div className="mt-8 space-y-5">{comments.filter((item) => item.status === "approved").slice(0, 3).map((item) => <div key={item.id} className="border-b border-[var(--color-border)] pb-5 last:border-0"><div className="flex items-center gap-2"><span className="flex h-7 w-7 items-center justify-center rounded-full bg-[var(--color-accent-main)]/15 text-xs font-bold text-[var(--color-accent-main)]">{item.avatar}</span><p className="text-sm font-semibold">{item.name}</p></div><p className="mt-2 pl-9 text-sm leading-6 text-[var(--color-text-secondary)]">"{item.message}"</p></div>)}</div><Link to="/contact" className="mt-6 inline-flex items-center gap-2 text-sm font-semibold text-[var(--color-accent-main)]">{t("Visit Contact & Guestbook")} <ArrowRight size={15} /></Link></Reveal>}</div></section>
    ...
    <section className="final-cta relative overflow-hidden border-t border-[var(--color-border)] bg-[var(--color-bg-secondary)] px-6 py-28">
      ...
      <Reveal className="relative mx-auto max-w-3xl text-center"><Eyebrow>{t("11 / Let's make it real")}</Eyebrow><h2 className="font-manrope text-5xl font-bold tracking-[-.035em] md:text-6xl">{t("Let's Build Something Meaningful.")}</h2><p className="mx-auto mt-5 max-w-xl leading-7 text-[var(--color-text-secondary)]">{t("A clear idea, a complex challenge, or an early sketch-the right next step can start with a conversation.")}</p><div className="mt-9 flex flex-wrap justify-center gap-3"><Link to="/contact" className="inline-flex items-center gap-2 bg-[var(--color-text-main)] px-5 py-3 text-sm font-bold text-[var(--color-bg-primary)]">{t("Start a Project")} <ArrowRight size={16} /></Link><Link to="/projects" className="border border-[var(--color-border)] px-5 py-3 text-sm font-bold hover:bg-white/5">{t("View All Projects")}</Link></div></Reveal></section>
